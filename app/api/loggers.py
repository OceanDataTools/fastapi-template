...
async def list_loggers(
    ...
    return await crud_loggers.get_loggers(session)
...
async def add_logger_configs_to_logger(
    logger_id: str,
    logger_config_ids: List[str],
    session: AsyncSession = Depends(get_async_session),
):
    try:
        return await crud_loggers.assign_logger_configs_to_logger(session, logger_id, logger_config_ids)
