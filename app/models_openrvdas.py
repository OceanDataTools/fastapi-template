...
# These tables are created by migration c1e2f3a4b5d6 (present on main/dev for
# historical reasons) but are only actually used by the `openrvdas` branch,
# which has its own richer version of this file. They're declared here purely
# so `alembic revision --autogenerate` on main/dev sees the schema as already
# accounted for, instead of proposing to drop these tables. See issue #12.
